---
name: openhue
description: "Control Philips Hue lights, scenes, rooms via OpenHue CLI — fallback only; prefer home-assistant-covers when the `ha` MCP is available."
version: 1.0.1
author: community
license: MIT
...
> (`mcp__ha__list_lights`, `mcp__ha__light_command`, `mcp__ha__activate_scene`
> — see the **`home-assistant-covers`** skill). Hue bulbs, rooms, and scenes
> already surface as ordinary `light.*`/`scene.*` entities through Home
> Assistant, which returns `confirmed` state and covers lights + covers +
> scenes in one interface; the `openhue` binary isn't installed in the
> sandbox and can't reach the Hue Bridge anyway.
>
> Keep this skill only for a direct-to-bridge scenario where HA is
> unavailable and `openhue` has been installed on the host by hand.
>
> Note: this CLI takes color temperature in **mirek** (153–500), while HA and
> the `ha` MCP use **Kelvin**. Don't mix the two.
