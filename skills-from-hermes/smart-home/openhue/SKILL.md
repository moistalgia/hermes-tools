...
> **⚠️ Not the preferred path in this house. Use the `ha` MCP server instead.**
>
> Hue bulbs, rooms, and scenes already surface as ordinary `light.*` / `scene.*`
> entities through Home Assistant. Use `mcp__ha__list_lights`,
> `mcp__ha__light_command`, and `mcp__ha__activate_scene` — see the
> **`home-assistant-covers`** skill.
>
> Why prefer HA here:
> - The `openhue` binary is **not installed** in the sandbox container, and the
>   container cannot reach the Hue Bridge on the LAN anyway.
> - The MCP returns `confirmed` by reading state back; this CLI does not, so an
>   agent can claim a light changed when it didn't.
> - One interface for covers + lights + scenes beats two.
> - No bridge pairing or extra credentials needed.
>
> Keep this skill only for a direct-to-bridge scenario where HA is unavailable
> and `openhue` has been installed on the host by hand.
>
> Note: this CLI takes color temperature in **mirek** (153–500), while HA and the
> `ha` MCP use **Kelvin**. Don't mix the two.
